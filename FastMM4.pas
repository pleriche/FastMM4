...
{Lock contention logging requires both FullDebugMode and ~ASMVersion.}
...
function FastGetMem(ASize: {$ifdef XE2AndUp}NativeInt{$else}Integer{$endif}
...
{$endif}
      ...
      begin
{$ifdef LogLockContention}
        if assigned(LCollector) then begin
          GetStackTrace(@LStackTrace, StackTraceDepth, 1);
